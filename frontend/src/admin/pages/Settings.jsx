...
const TABS = [
  ...
  { id: "hero", label: "Hero & About", icon: Type },
...
export default function Settings() {
  ...
  const save = async () => {
    if (!data.phone?.trim() || !data.email?.trim()) {
      toast.error("Telefon va email to'ldirilishi shart");
      return;
    }
    setSaving(true);
    try {
      ...
      <div className="rounded-2xl bg-white p-6 shadow-soft ring-1 ring-black/5">
        {tab === "contact" && (
          <div className="grid max-w-2xl grid-cols-1 gap-5 sm:grid-cols-2">
            <Input label="Telefon" icon={Phone} value={data.phone || ""} onChange={(e) => set("phone", e.target.value)} />
            <Input label="Email" value={data.email || ""} onChange={(e) => set("email", e.target.value)} />
            <Input label="Manzil" icon={MapPin} value={data.address || ""} onChange={(e) => set("address", e.target.value)} className="sm:col-span-2" />
            <Input label="Ish vaqti" value={data.working_hours || ""} onChange={(e) => set("working_hours", e.target.value)} className="sm:col-span-2" />
          </div>
        )}

        {tab === "hero" && (
          <div className="flex max-w-2xl flex-col gap-5">
            <Input label="Hero belgisi (badge)" value={data.hero_badge || ""} onChange={(e) => set("hero_badge", e.target.value)} />
            <Input
              label="Hero sarlavha"
            ...
            <p className="-mt-3 text-xs text-neutral-400">
              Gradient so'z uchun **yulduzcha** ichiga oling: masalan, **professional**
            </p>
            <Textarea label="Hero tavsif" rows={3} value={data.hero_subtitle || ""} onChange={(e) => set("hero_subtitle", e.target.value)} />
            <Textarea label="Biz haqimizda matni" rows={4} value={data.about_text || ""} onChange={(e) => set("about_text", e.target.value)} />
          </div>
        )}
        ...
        {tab === "video" && (
          <div className="flex max-w-2xl flex-col gap-5">
            <Input
              label="Asosiy video (YouTube havolasi)"
              placeholder="https://www.youtube.com/watch?v=..."
              value={data.video_url || ""}
              onChange={(e) => set("video_url", e.target.value)}
            />
            <p className="-mt-3 text-xs text-neutral-400">
              Saytdagi video bo'limida ko'rsatiladigan asosiy YouTube video havolasi.
            </p>
          </div>
        )}
      </div>
